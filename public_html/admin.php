...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    try {
        ...
        if ($action === 'save') {
            ...
            if ($type === 'hero') $record = ['id'=>$id,'title'=>trim((string)$_POST['title']),'lead'=>trim((string)$_POST['lead']),'color'=>(string)$_POST['color'],'image'=>upload_image('image',(string)($existing['image']??''),'hero/'.$id),'overlay'=>(string)$_POST['overlay'],'overlay_opacity'=>max(0,min(100,(int)$_POST['overlay_opacity'])),'dots'=>isset($_POST['dots']),'dots_opacity'=>max(0,min(100,(int)$_POST['dots_opacity'])),'published'=>isset($_POST['published'])];
            elseif ($type === 'works') $record = ['id'=>$id,'title'=>trim((string)$_POST['title']),'category'=>trim((string)$_POST['category']),'area'=>trim((string)$_POST['area']),'summary'=>trim((string)$_POST['summary']),'latitude'=>(float)$_POST['latitude'],'longitude'=>(float)$_POST['longitude'],'image'=>upload_image('image',(string)($existing['image']??''),'works/'.$id),'published'=>isset($_POST['published'])];
...
?><!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>管理画面｜<?= APP_NAME ?></title><link rel="stylesheet" href="assets/admin.css"><link rel="stylesheet" href="assets/admin-dashboard.css?v=2"></head><body class="admin-shell">
...
<div class="dashboard-panels">
  <section class="panel inquiry-chart">
  ...
  <section class="panel site-operation-guide">
    <h2>サイト運用ガイド</h2>
    <ul>
      <li><strong>主要セクションを管理画面から更新</strong><span>左メニューからHERO、ホーム、強み、サービス、地図、施工事例、最新情報、会社情報を編集できます。</span></li>
      <li><strong>画像を自動で最適化</strong><span>JPEG・PNG・WebP、1枚25MBまで対応。大きな画像は比率を保ち、最大1920pxへ自動縮小・圧縮します。</span></li>
      <li><strong>公開状態を安全に管理</strong><span>公開／下書きを切り替え、内容を確認してから公開サイトへ反映できます。</span></li>
      <li><strong>関連データを連携</strong><span>地図と施工事例、サービスカードと詳細ページなど、関連する情報をまとめて管理できます。</span></li>
    </ul>
  </section>
</div>
