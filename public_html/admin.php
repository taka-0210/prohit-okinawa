...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    try {
        ...
        if ($action === 'save') {
            ...
            else $record = ['id'=>$id,'title'=>trim((string)$_POST['title']),'category'=>trim((string)$_POST['category']),'body'=>trim((string)$_POST['body']),'published_at'=>(string)$_POST['published_at'],'published'=>isset($_POST['published'])];
...
$tab = in_array($_GET['tab'] ?? 'dashboard',['dashboard','hero','works','news'],true) ? $_GET['tab'] : 'dashboard';
...
<?php if($tab==='dashboard'): $counts=['hero'=>load_content('hero'),'works'=>load_content('works'),'news'=>load_content('news')]; ?><div class="stats"><?php foreach(['hero'=>'HERO','works'=>'施工事例','news'=>'最新情報'] as $key=>$label):?><a href="?tab=<?=$key?>"><span><?=count($counts[$key])?></span><?=$label?><small>公開 <?=count(published($counts[$key]))?>件</small></a><?php endforeach;?></div><section class="panel"><h2>運用の中心</h2><ol><li>HERO画像と見え方を調整</li><li>施工事例を座標付きで登録</li><li>最新情報を公開・更新</li></ol><p>画像はJPEG・PNG・WebP、6MBまで。公開前に会社情報と問い合わせ先を確定してください。</p></section>
