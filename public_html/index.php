...
<section class="section works" id="works"><p class="section-no">03 / WORKS</p><div><div class="section-head"><h2>現場から生まれた、<br>お店づくりの実績。</h2><span>OKINAWA PROJECTS</span></div><div class="work-list"><?php foreach(array_slice(array_reverse($works),0,4) as $work):$workImages=work_images($work);?><article><?php if($workImages):?><button class="work-gallery-trigger" type="button" data-title="<?=e($work['title'])?>" data-images="<?=e(json_encode($workImages,JSON_UNESCAPED_SLASHES))?>"><span class="placeholder" style="background-image:url(<?=e($workImages[0])?>)"></span><span class="photo-count"><?=count($workImages)?> PHOTOS</span></button><?php else:?><div class="placeholder"></div><?php endif;?><p><?= e($work['category']) ?> / <?= e($work['area']) ?></p><h3><?= e($work['title']) ?></h3><p><?= e($work['summary']) ?></p></article><?php endforeach; ?></div><a class="button works-more" href="works.php">地図と施工事例をもっと見る →</a></div></section>
...
<section class="company section" id="company"><p class="section-no">06 / COMPANY</p><div><div class="company-heading"><div><p class="eyebrow">COMPANY PROFILE</p><h2><?=e($company['company_name']??APP_NAME)?></h2><p><?=nl2br(e($company['description']??''))?></p></div><p class="company-en"><?=e($company['company_name_en']??'PRO CHUBO HIT OKINAWA')?></p></div><div class="company-columns"><dl><div><dt>所在地</dt><dd><?=e(trim(($company['postal_code']??'').' '.($company['address']??'')))?></dd></div><div><dt>代表取締役</dt><dd><?=e($company['representative']??'')?></dd></div><?php if(!empty($company['phone'])):?><div><dt>電話</dt><dd><?=e($company['phone'])?></dd></div><?php endif;?></dl><div class="history"><h3>HISTORY <small>沿革</small></h3><?php foreach($history as $row):?><article><time><?=e($row['date'])?></time><p><?=e($row['detail'])?></p></article><?php endforeach;?></div></div></div></section>
<section class="news section" id="news"><p class="section-no">07 / NEWS</p><div><h2>最新情報</h2><?php foreach(array_slice($news,0,3) as $item): ?><a class="home-news-link" href="news-detail.php?id=<?= rawurlencode((string)($item['id']??'')) ?>"><article><time><?= e($item['published_at']) ?></time><span><?= e($item['category']) ?></span><h3><?= e($item['title']) ?></h3></article></a><?php endforeach; ?><a class="button news-more" href="news.php">最新情報をもっと見る →</a></div></section>
<section class="instagram-section section" id="instagram"><p class="section-no">08 / INSTAGRAM</p><div><div class="instagram-heading"><div class="instagram-account"><?php if(!empty($company['logo'])):?><img src="<?=e($company['logo'])?>" class="instagram-account-thumb" id="profile-okinawa" alt="プロ厨房HIT 沖縄営業所"><?php else:?><span class="instagram-account-thumb" id="profile-okinawa"></span><?php endif;?><div><small>OFFICIAL INSTAGRAM</small><h2>プロ厨房HIT 沖縄営業所</h2></div></div></div><div class="swiper instagram-swiper swiper-okinawa"><div class="swiper-wrapper" id="feed-okinawa"><p class="instagram-loading">投稿を読み込んでいます。</p></div><button class="swiper-button-prev swiper-button-okinawa-prev" type="button" aria-label="前の投稿"></button><button class="swiper-button-next swiper-button-okinawa-next" type="button" aria-label="次の投稿"></button></div></div></section>
