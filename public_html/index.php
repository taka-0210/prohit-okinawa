...
<section class="story section strength-section" id="strength" style="--strength-duration:<?=max(30,min(180,(int)($homeStrength['scroll_duration']??72)))?>s"><p class="section-no">02 / OUR STRENGTH</p><div class="strength-copy"><h2><?=nl2br(e($homeStrength['heading']??''))?></h2><?php foreach(preg_split('/\R{2,}/u',trim((string)($homeStrength['body']??'')))?:[] as $paragraph): ?><?php if(preg_match('/^##\s+(.+)$/us',trim($paragraph),$headingMatch)):?><h3><?=nl2br(e($headingMatch[1]))?></h3><?php else:?><p><?=nl2br(e($paragraph))?></p><?php endif;?><?php endforeach; ?></div><?php $strengthImages=array_values(array_filter((array)($homeStrength['images']??[]),'is_string'));if($strengthImages):?><div class="strength-reel" aria-label="OUR STRENGTH 写真"><div class="strength-reel-track"><?php for($copy=0;$copy<4;$copy++):?><div class="strength-reel-group" <?=$copy>0?'aria-hidden="true"':''?>><?php foreach($strengthImages as $image):?><img src="<?=e($image)?>" alt="" loading="lazy"><?php endforeach;?></div><?php endfor;?></div></div><?php endif;?><div class="strength-quote"><blockquote><?=nl2br(e($homeStrength['quote']??''))?></blockquote></div></section>
...
<?php foreach($servicePages as $index=>$servicePage): $serviceCardImage=(string)($servicePage['card_image']??'');$serviceCardStrength=max(10,min(80,(int)($servicePage['card_image_opacity']??40)));$serviceCardOverlay=(100-$serviceCardStrength)/100;$serviceCardOverlayBottom=min(.9,$serviceCardOverlay+.1);?><article data-service-slug="<?=e($servicePage['id'])?>"<?=$serviceCardImage!==''?' class="has-service-image" style="background-image:linear-gradient(rgba(16,42,67,'.number_format($serviceCardOverlay,2,'.','').'),rgba(16,42,67,'.number_format($serviceCardOverlayBottom,2,'.','').')),url(\''.e($serviceCardImage).'\')"':''?>><span><?=str_pad((string)($index+1),2,'0',STR_PAD_LEFT)?></span><h3><?=e($servicePage['title'])?></h3><p><?=e($servicePage['lead'])?></p></article><?php endforeach; ?>
</div></div></section>
...
<section class="company section" id="company"><p class="section-no">05 / COMPANY</p><div><div class="company-heading"><div><p class="eyebrow">COMPANY PROFILE</p><h2><?=e($company['company_name']??APP_NAME)?></h2><p><?=nl2br(e($company['description']??''))?></p></div><p class="company-en"><?=e($company['company_name_en']??'PRO CHUBO HIT OKINAWA')?></p></div><?php $companyPhotos=array_filter(['EXTERIOR'=>$company['exterior_image']??'','INTERIOR'=>$company['interior_image']??'']);if($companyPhotos):?><div class="company-photos"><?php foreach($companyPhotos as $label=>$image):?><figure><img src="<?=e($image)?>" alt="<?=e($company['company_name']??APP_NAME)?> <?=e($label==='EXTERIOR'?'外観':'内観')?>" loading="lazy"><figcaption><?=e($label)?></figcaption></figure><?php endforeach;?></div><?php endif;?><div class="company-columns"><dl><div><dt>所在地</dt><dd><?=e(trim(($company['postal_code']??'').' '.($company['address']??'')))?></dd></div><div><dt>代表取締役</dt><dd><?=e($company['representative']??'')?></dd></div><?php if(!empty($company['phone'])):?><div><dt>電話</dt><dd><?=e($company['phone'])?></dd></div><?php endif;?></dl><div class="history"><h3>HISTORY <small>沿革</small></h3><?php foreach($history as $row):?><article><time><?=e($row['date'])?></time><p><?=e($row['detail'])?></p></article><?php endforeach;?></div></div></div></section>
